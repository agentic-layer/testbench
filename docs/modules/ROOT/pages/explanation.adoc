...
**Testkube** — Testbench is implemented as a set of reusable `TestWorkflowTemplate` CRDs installed by the bundled `install.yaml`. The Testbench operator reconciles each user-authored `Experiment` resource into a Testkube `TestWorkflow` that composes those templates. Testkube orchestrates the pipeline phases, provides the shared volume that passes state between phases, exposes workflow triggers (e.g. on every agent redeployment), and stores artifacts such as the HTML report. Testbench does not implement any scheduling or execution logic of its own.
...
Testkube provides workflow orchestration, artifact storage, trigger support, and a UI for browsing execution history at no extra infrastructure cost (it is already required by the Agentic Layer platform). Building a custom evaluation runner would replicate this infrastructure. The trade-off is that Testbench is tightly coupled to the Testkube CRD API: the operator runs in its own `testbench-operator-system` namespace, but the `TestWorkflowTemplate` resources it relies on (and the `TestWorkflow` / `TestTrigger` resources it generates) live in the `testkube` namespace alongside the Testkube controller.
