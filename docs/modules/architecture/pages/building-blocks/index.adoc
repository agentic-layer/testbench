...
Field-level details live in xref:testbench::reference.adoc[Reference].
...
System_Boundary(testbench, "Testbench Pipeline") {
    ...
    Container(visualize, "Phase 5: Visualize", "Python", "Generates self-contained\nHTML dashboard artifact")
}
...
. Parses CSV, JSON, or Parquet input into the canonical `Experiment` JSON model
. Writes `data/datasets/experiment.json` to the shared data volume
